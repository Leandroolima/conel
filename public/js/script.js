class Cadastro {
    ...
    async visualizar(id_cadastrado) {
        fetch('35.232.172.68/cadastros/' + id_cadastrado)
            .then(result => {
                return result.json()
            }).then(data => {
                for (let i = 0; i < this.arrayCadastros.length; i++) {
                    if (this.arrayCadastros[i].id_cadastrado == id_cadastrado) {
                        window.location.assign("35.232.172.68" + data.cadastrado.arquivo)
...
class Denuncia {
    ...
    async visualizar(id_denuncia) {
        fetch('35.232.172.68/denuncias/' + id_denuncia)
            .then(result => {
                return result.json()
            }).then(data => {
                for (let i = 0; i < this.arrayDenuncia.length; i++) {
...
class Trabalhe {
    ...
    async visualizar(id_trabalhe) {
        fetch('35.232.172.68/trabalhe/' + id_trabalhe)
            .then(result => {
                return result.json()
            }).then(data => {
                for (let i = 0; i < this.arrayTrabalhe.length; i++) {
...
class Login {
    ...
    async adicionar(login) {
        fetch('35.232.172.68/usuario/login', {
            method: 'POST',
